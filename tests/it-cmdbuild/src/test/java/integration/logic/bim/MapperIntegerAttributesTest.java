...
public class MapperIntegerAttributesTest extends IntegrationTestBimBase {
	...
	@Test
	public void createCardWithIntegerAttribute() throws Exception {
		// given
		...
		attributeList.add(new BimAttribute(ATTRIBUTE_NAME, "10"));
		source.add(e);
		...
		assertThat(card.get(ATTRIBUTE_NAME).toString(), equalTo("10"));
	}

	@Test
	public void updateCardWithIntegerAttribute() throws Exception {
		// given
		final String codeEdificio = "E" + RandomStringUtils.random(5);
		final String globalId = RandomStringUtils.random(22);
		List<Entity> source = Lists.newArrayList();
		...
		attributeList.clear();
		attributeList.add(new BimAttribute("GlobalId", globalId));
		attributeList.add(new BimAttribute(ATTRIBUTE_NAME, "9"));
		...
		assertThat(card.getDescription().toString(), equalTo("Edificio 1"));
		assertThat(card.getCode().toString(), equalTo(codeEdificio));
		assertThat(card.get(ATTRIBUTE_NAME).toString(), equalTo("9"));
	}
