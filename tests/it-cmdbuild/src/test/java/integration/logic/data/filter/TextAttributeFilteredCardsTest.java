...
public class TextAttributeFilteredCardsTest extends FilteredCardsFixture {
	...
	public void fetchFilteredCardsWithEqualOperator() throws Exception {
		...
		// when
		final Iterable<CardDTO> fetchedCards1 = dataAccessLogic.fetchCards(createdClass.getName(),
				queryOptionsForEmptyResult).getPaginatedCards();
		final Iterable<CardDTO> fetchedCards2 = dataAccessLogic.fetchCards(createdClass.getName(), queryOptions)
				.getPaginatedCards();
	...
	public void resultsOfContainUnionNotContainMustBeComplementary() throws Exception {
		...
		// when
		final Iterable<CardDTO> notContainFetchedCards = dataAccessLogic.fetchCards(createdClass.getName(),
				notContainQueryOptions).getPaginatedCards();
		final Iterable<CardDTO> containFetchedCards = dataAccessLogic.fetchCards(createdClass.getName(),
