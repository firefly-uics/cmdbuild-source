...
public class BimserverListAttribute extends BimserverAttribute implements ListAttribute {
	...
	public BimserverListAttribute(final SListDataValue value) {
		super(value);
	}

	@Override
	public List<Attribute> getValues() {
		final List<SDataValue> datavalues = ((SListDataValue) getDatavalue()).getValues();
		...
		for (final SDataValue datavalue : datavalues) {
			if (datavalue instanceof SSimpleDataValue) {
				final Attribute attribute = new BimserverSimpleAttribute((SSimpleDataValue) datavalue);
				values.add(attribute);
			} else if (datavalue instanceof SListDataValue) {
				final Attribute attribute = new BimserverListAttribute((SListDataValue) datavalue);
				values.add(attribute);
			} else if (datavalue instanceof SReferenceDataValue) {
				final Attribute attribute = new BimserverReferenceAttribute((SReferenceDataValue) datavalue);
				values.add(attribute);
			}
		}
		return values;
