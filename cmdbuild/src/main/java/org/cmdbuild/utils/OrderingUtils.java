...
public class OrderingUtils {
	...
	static public <T> void alterPosition(List<? extends T> list, final int oldPos, final int newPos, final PositionHandler<T> positionHandler) {
		...
		for (T item : list) {
			...
			int currentPosition = positionHandler.getPosition(item);
