...
public class RelationFactoryParameter extends AbstractParameterBuilder<RelationFactory> {
	...
	@Override
	public RelationFactory build(final HttpServletRequest r) throws AuthException, ORMException, NotFoundException {
		final UserContext userCtx = new SessionVars().getCurrentUserContext();
		return UserOperations.from(userCtx).relations();
	}
}
