...
public class XMLSerializer {
	...
	public static Document serializeCard(ICard card) {
		Document document = DocumentHelper.createDocument();
		Element xmlCard = document.addElement("data").addElement("card");
		for(AttributeValue attributeValue : card.getAttributeValueMap().values()) {
			IAttribute attribute = attributeValue.getSchema();
			...
			item.addAttribute("realName", attribute.getName());
			item.addAttribute("name", attribute.getDescription());
			item.addAttribute("value", valueForGraph);
		}
		return document;
	}

	public static Document serializeGraph(
			Set<GraphNode> nodes,
			Set<GraphEdge> edges,
			Set<GraphRelation> relationGroups) {
		Document document = DocumentHelper.createDocument();
	...
	private static Element serializeRelations(
			Set<GraphRelation> relationGroups) {
	...
	private static Element serializeGraphMLNode(
			Set<GraphNode> nodes,
			Set<GraphEdge> edges) {
	...
	private static Element serializeGraphNode(
			Set<GraphNode> nodes,
			Set<GraphEdge> edges) {
	...
	private static Element genGraphKey(
			String id, boolean node, String type) {
