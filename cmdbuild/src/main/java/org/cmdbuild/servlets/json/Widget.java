...
public class Widget extends JSONBase {
	...
	public JsonResponse callWidget(
			...
			@Parameter("className") final String className,
	...
	public JsonResponse callWidget(
			UserContext userCtx,
			@Parameter("id") final int processCardId,
			@Parameter("className") final String className,
			@Parameter("activityId") final String activityInstanceId,
			@Parameter("widgetId") final String widgetId,
			@Parameter(required = false, value = "action") final String action,
			@Parameter(required = false, value = "params") final String jsonParams) throws Exception {
		...
		if (processCardId > 0) {
			final CMActivityInstance activityInstance = logic.getActivityInstance(className, new Long(processCardId), activityInstanceId);
			widgets = activityInstance.getWidgets();
		}	else {
			// For a new process, there isn't activity instances. So retrieve the start activity
			// and look for them widgets
			CMActivity  activity = logic.getStartActivity(className);
			widgets = activity.getWidgets();
		}

		for (CMActivityWidget widget : widgets) {
			if (widget.getId().equals(widgetId)) {
				response = widget.executeAction(action, params, null); 	// TODO I don't know WTF pass instead of null,
	...
	private Map<String, Object> readParams(final String jsonParams)
