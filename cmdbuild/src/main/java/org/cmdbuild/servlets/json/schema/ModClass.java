...
public class ModClass extends JSONBase {
	...
	public JSONObject getFieldModes(final JSONObject serializer) throws JSONException, AuthException {
		for (final FieldMode type : FieldMode.values()) {
	...
	@JSONExported
	public JSONObject saveTable(final JSONObject serializer,
			...
			@Parameter("userstoppable") final boolean isProcessUserStoppable, final ITable table) throws JSONException,
			CMDBException {
		...
		table.save();
		final JSONObject result = Serializer.serializeTable(table);
	...
	@JSONExported
	public JSONObject saveAttribute(final JSONObject serializer,
			@Parameter(value = "name", required = false) final String name,
			...
			@Parameter("isbasedsp") final boolean isBaseDSP, @Parameter("isnotnull") final boolean isNotNull,
			@Parameter("isunique") final boolean isUnique, @Parameter("isactive") final boolean isActive,
			...
			final BaseSchema table, final UserContext userCtx) throws JSONException, CMDBException {
		...
		if (attributeTypeString.equals(AttributeType.REFERENCE.toString())) {
			...
			if (fkDestinationId > 0) {
				attribute.setFKTargetClass(UserOperations.from(userCtx).tables().get(fkDestinationId).getName());
			}
		}
		attribute.save();
		if (meta != null) {
	...
	@JSONExported
	public JSONObject deleteAttribute(final JSONObject serializer, @Parameter("name") final String attributeName,
			final BaseSchema table) throws JSONException {
		final IAttribute attribute = table.getAttribute(attributeName);
		try {
			attribute.delete();
		} catch (final ORMException e) {
			if (e.getExceptionType() == ORMExceptionType.ORM_CONTAINS_DATA) {
