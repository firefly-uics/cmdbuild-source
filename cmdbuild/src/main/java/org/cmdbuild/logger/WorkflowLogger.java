...
public class WorkflowLogger implements CMWorkflowEngineListener {
	...
	public void syncProcessInstanceNotFound(CMProcessInstance processInstance) {
		if (Log.WORKFLOW.isInfoEnabled()) {
			Log.WORKFLOW.info(String.format("Process instance %s not found: removing card %s", processInstance.getProcessInstanceId(), processInstance.getCardId()));
		}
	}
	...
	public void syncProcessInstanceFound(CMProcessInstance processInstance) {
		if (Log.WORKFLOW.isDebugEnabled()) {
			Log.WORKFLOW.debug(String.format("Process instance %s found: updating card %s", processInstance.getProcessInstanceId(), processInstance.getCardId()));
		}
	}
