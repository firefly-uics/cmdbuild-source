...
public class AuthenticatedUserWrapper extends UserContext {
	...
	private class PrivilegeManagerWrap implements PrivilegeManager {
		...
		@Override public PrivilegeType getPrivilege(BaseSchema schema) {
		...
		@Override public boolean hasReadPrivilege(BaseSchema schema) { return user.hasPrivilege(DefaultPrivileges.READ, schema); }
		private boolean hasWritePrivilege(BaseSchema schema) { return user.hasPrivilege(DefaultPrivileges.WRITE, schema); }

		@Override public boolean hasReadPrivilege(ITable table) { return user.hasPrivilege(DefaultPrivileges.READ, table); }
		@Override public boolean hasWritePrivilege(ITable table) { return user.hasPrivilege(DefaultPrivileges.WRITE, table); }
		@Override public void assureReadPrivilege(ITable table) { assurePrivilege(DefaultPrivileges.READ, table); }
		@Override public void assureWritePrivilege(ITable table) { assurePrivilege(DefaultPrivileges.WRITE, table); }
		...
		@Override public boolean hasReadPrivilege(IDomain domain) { return user.hasPrivilege(DefaultPrivileges.READ, domain); }
		@Override public boolean hasWritePrivilege(IDomain domain) { return user.hasPrivilege(DefaultPrivileges.WRITE, domain); }
		@Override public void assureReadPrivilege(IDomain domain) { assurePrivilege(DefaultPrivileges.READ, domain); }
		@Override public void assureWritePrivilege(IDomain domain) { assurePrivilege(DefaultPrivileges.WRITE, domain); }
		...
		@Override public boolean hasCreatePrivilege(ITable table) {
			return table.getMode().cardsAllowed() && hasWritePrivilege(table);
		}
		@Override public boolean hasCreatePrivilege(IDomain domain) {
			return hasWritePrivilege(domain);
		}

		@Override public boolean isAdmin() { return user.hasAdministratorPrivileges(); }
		@Override public void assureAdminPrivilege() { if (!isAdmin()) throw AuthExceptionType.AUTH_NOT_AUTHORIZED.createException(); }
	}
