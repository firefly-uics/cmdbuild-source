...
public class GeoFeatureStore {
	...
	public GeoFeatureStore (final DataSource dataSource) {
	...
	private static final String GEO_TABLE_COMMENT_TEMPLATE = "DESCR: %s|MODE: write|STATUS: active|SUPERCLASS: false|TYPE: simpleclass"; // attribute description
	private static final String CREATE_GEO_TABLE_TEMPLATE = "SELECT cm_create_class('%s', NULL, '%s')"; // class name, comment
	private static final String DELETE_GEO_TABLE_TEMPLATE = "SELECT cm_delete_class('%s')"; // class name
	...
	// cm_create_class_attribute(cmclass text, attributename text, sqltype text, attributedefault text,
	...
	public String createGeoTable( //
		...
		final String geoAttributeTableName = String.format(GEO_TABLE_NAME_FORMAT, targetClassName, layerMetadata.getName());
		final String geoAttributeTableComment = String.format(GEO_TABLE_COMMENT_TEMPLATE, layerMetadata.getDescription());
		final String createGeoTableQuery = String.format(CREATE_GEO_TABLE_TEMPLATE, geoAttributeTableName, geoAttributeTableComment);
	...
	public GeoFeature readGeoFeature( //
		...
		jdbcTemplate.query(readGeoFeatureQuery(
				layerMetaData, //
				ownerCard.getId() //
	...
	private String createGeoFeatureQuery(
			final LayerMetadata layerMetaData, //
			final String value, //
			final Long ownerCardId
			) {
	...
	private String readGeoFeatureQuery(
		final LayerMetadata layerMetaData, //
		final Long ownerCardId //
		) { //
	...
	private String deleteGeoFeatureQuery(
			final LayerMetadata layerMetaData,
			final Long ownerCardId
			) {
	...
	private String readGeoFeaturesQuery(
			final LayerMetadata layerMetaData, //
			final String bbox //
			) {
	...
	private String readGeoFeatureQueryFromPart(final LayerMetadata layerMetadata) {
		final String fromTemplate = "FROM %s AS \"%s\" " +
				"JOIN \"%s\" AS \"%s\" " +
				"ON \"%s\".\"%s\" = \"%s\".\"%s\" ";
	...
	private String getGeoFeaturesQueryWherePart(final String bbox) {
		...
		final String postGISVersion = DBService.getPostGISVersion();
