...
public class ReportQuery {
	...
	private static final String INSERT_TEMPLATE =
			...
			"\"ImagesName\")"+
			" VALUES (?,?,?,?,?,?,?,?,?,?,?," +
			"cast(string_to_array('%s',',') as int[])," +
			"cast(string_to_array('%s',',') as int[])," +
			"cast(string_to_array('%s',',') as varchar[])," +
			"cast(string_to_array('%s',',') as varchar[]));";
	...
	public static final QueryConfiguration selectById(final int id) {
		return new QueryConfiguration(SELECT_BY_ID_TEMPLATE, new Object[] {
			id
		});
	}

	public static final QueryConfiguration delete(int id) {
	...
	public static final QueryConfiguration update(final Report report) {
		...
		if (report.getJd() != null) {
			...
			query.append(String.format("\"ReportLength\" = cast(string_to_array('%s',',') as int[]),", arrayToCsv(report.getReportLength())));
			query.append(String.format("\"ImagesLength\" = cast(string_to_array('%s',',') as int[]),", arrayToCsv(report.getImagesLength())));
			query.append(String.format("\"ImagesName\" = cast(string_to_array('%s',',') as varchar[]) ", arrayToCsv(report.getImagesName())));
...
//	+ Report.REPORT_CLASS_NAME
	private static String arrayToCsv(Object array) {
