...
import org.cmdbuild.dao.view.SystemClass;
...
public class MenuItemConverter {
	 ...
	 * @param groupName
	 * @param menuItem
	 * @return a menuCard for the given menuItem to assign to the given group name
	 */
	public static MenuCard toMenuCard(String groupName, MenuItem menuItem) {
	 ...
	 */
	public static MenuItem fromMenuCard(Iterable<MenuCard> menuCardList) {
		...
		for (MenuCard menuCard: menuCardList) {
			Integer id = menuCard.getId();
			items.put(id, convertMenuCardToMenuItemBuilder(menuCard));
		}

		for (ConvertingItem item: items.values()) {
	 ...
	 */
	public static MenuItem fromCMClass(CMClass cmClass) {
	...
	public static MenuItem fromCMReport(CMCard report, ReportExtension extension) {
		final MenuItem menuItem = new MenuItemDTO();
	...
	public static MenuItem fromDashboard(DashboardDefinition dashboardDefinition, Long id) {
		final MenuItem menuItem = new MenuItemDTO();

		menuItem.setType(MenuItemType.DASHBOARD);
		menuItem.setReferedClassName("_Dashboard"); // FIXME retrieve the name from a constant
	...
	private static ConvertingItem convertMenuCardToMenuItemBuilder(MenuCard menuCard) {
		final MenuItem menuItem = new MenuItemDTO();
		...
		String className = UserOperations.from(UserContext.systemContext()).tables()
	...
	MenuCard fromMenuItemToMenuCard(String groupName, MenuItem menuItem) {
	...
	private static class EntryTypeConverterStrategy extends MenuItemConverter {
		...
		MenuCard fromMenuItemToMenuCard(String groupName, MenuItem menuItem) {
			...
			menuCard.setElementClassId(UserOperations
					.from(UserContext.systemContext()).tables()
	...
	private static class ConvertingItem {
		...
		public ConvertingItem(MenuCard menuCard, MenuItem menuItemBuilder) {
