 ...
 * Basically, used in conjunction with the CQLCompilerBuilder, is a Builder for Query objects.
 */
public class CQLCompilerListener extends CQLBuilderAdaptor {
	
	/**
	 * used to generate custom AS for, e.g., domains defined in the WHERE statement
	 */
	...
	/**
	 * Hold the parser state for the current Expression, needed for nested expressions.
	 */
	...
	QueryScope cur(){return scope.peek();}
	void push(){scope.push(new QueryScope());}
	void pop(){scope.pop();}

	Query curQuery(){return cur().query;}
	WhereElement curWhere(){return cur().wheres.peek();}
	
	public void setFactory(AbstractElementFactory factory){this.factory = factory;}
	...
	private void startFromDomain(String scopeReference, 
		...
		if(scopeReference == null) {
			...
			if(parent == null){
				throw new RuntimeException("Scope reference " + scopeReference + " for domain " + domainName + ":" + domainId + " as " + domainAs + " was not found!");
			}
	...
	private void createField(
		FieldId id,
		WhereType type, boolean isNot, String classOrDomainNameOrRef,
