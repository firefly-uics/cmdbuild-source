(function($) {
	var lookupField = function(param) {
		...
		this.init = function() {
			try {
				if (this.param.value) {
					this.lookupValue[this.param.lookupName] = this.param.value;
				}
				this.generateLookupTypeTree(this.param.lookupName);
			}
			catch (e) {
				$.Cmdbuild.errorsManager.log("$.Cmdbuild.standard.lookupField.init");
				throw e;
			}
		};
		...
		this.generateHTML = function() {
			...
			var lookupType = this.param.lookupName;
			...
			if (! this.lookupTree[lookupType].parent) {
				...
				this.addParentSelect(this.lookupTree[this.param.lookupName].parent, selectfield);
			}
		};
		...
		this.addOptions = function(selectfield, values) {
			...
			$.each(values, function(index, value) {
				var opt = $("<option>").attr("value", value._id).attr("entryValue", value._id);
