...
//---------------------------------------------
	...
	var ViewerUtilities =  {
		...
		webGlRender: function(container) {
			...
			renderer.setPixelRatio(1.);//container.devicePixelRatio);
		...
		spacePlane: function(visible, w, h) {
			w = (w) ? w : 12000;
			h = (h) ? h : 12000;
			var plane = new THREE.Mesh(new THREE.PlaneBufferGeometry(w, h, 8,
		...
		text: function(str, position, camera) {
			var size = 108;
			var curveSegments = 3;
			var text = new THREE.TextGeometry(str, {font: "helvetiker",
				size: 10,
				height: 3,
			});
...
//			});
		...
		objectFromNode: function(node, selected, position) {
			var sprite = $.Cmdbuild.SpriteArchive.class2Sprite($.Cmdbuild.g3d.Model.getGraphData(node, "className"));
			try {
				//var map = THREE.ImageUtils.loadTexture(sprite);
				...
				if (! map) {
					console.log("Error on Sprite: " + $.Cmdbuild.g3d.Model.getGraphData(node, "className") + " the sprite is " + sprite);
				}
				var material = new THREE.SpriteMaterial( { map: map, color: 0xffffff, fog: false } );
		...
		moveObject: function(viewer, node) {
			...
			this.TM = setInterval(function() { 
				...
				if (++i > STEPS) {
					...
					if (nextPosition) {
						new $.Cmdbuild.g3d.ViewerUtilities.moveObject(viewer, nextPosition);
					}
				}
			}, INTERVAL);
		},
		modifyLine: function(scene, edge, p1, p2) {
			...
			edge.glLine.geometry.vertices.push(new THREE.Vector3(p1.x, p1.y, p1.z));
			edge.glLine.geometry.vertices.push(new THREE.Vector3(p2.x, p2.y, p2.z));
		...
		//these can depends on the type (coass, shape, ?) of the object
