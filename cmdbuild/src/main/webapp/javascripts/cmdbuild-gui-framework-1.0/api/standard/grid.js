	...
	var grid = function() {
		...
		this.init = function(param) {
			...
			if (param.selectedItemId) {
				var positionOf = $.Cmdbuild.dataModel.resolveVariable(param.selectedItemId);
				param.positionOf = positionOf;
			}

			var backendFn = $.Cmdbuild.utilities.getBackend(param.backend);
			this.backend = new backendFn(param, this.chargeFilter, this);
		};
		...
		this.show = function(filterCql) {
			...
			try {
				...
				var sortDirection = param.direction || "asc";
...
//					var data = table.row(row).data();
		...
		this.getFieldValue = function(card, attribute) {
			...
			else if (["date", "timestamp","dateTime"].indexOf(attribute.type) != -1) {
				return $.Cmdbuild.utilities.convertDateDB2GUI(card[attribute.name]);
			}
			else {
