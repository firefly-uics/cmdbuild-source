(function($) {
	var commands = {
		...
		dialogExec: function(param) {
			$.Cmdbuild.standard.referenceField.onChangeLookup(param);
		...
		fieldChanged: function(param) {
			var fieldName = $('#' + param.id).attr("fieldName");
			param.fieldName = fieldName || param.id;
			param.formName = $('#' + param.id).attr("formName");
			$.Cmdbuild.CqlManager.fieldChanged(param.formName, param.fieldName);
		},
		...
		refreshMail : function(params) {
			...
			var widgets = formDataObject.param.backend.widgets;
		...
		downloadAttachment: function(params){
			...
			var nameProcess = $.Cmdbuild.dataModel
					.resolveVariable(params.className);
			var idProcess = $.Cmdbuild.dataModel.resolveVariable(params.cardId);
			var nameAttachment = $.Cmdbuild.dataModel
					.resolveVariable(params.attachmentName);
			var idAttachment = $.Cmdbuild.dataModel
			...
			var url = $.Cmdbuild.global.getApiUrl() 
				...
				+ "/"
				+ nameAttachment
				+ "?CMDBuild-Authorization=" + $.Cmdbuild.authentication.getAuthenticationToken();
