(function($) {
	...
	var form = function() {
		...
		this.flush = function(param, callback, callbackScope) {
			this.param.backend.updateData(param, callback, callbackScope);
		};
		this.refreshField = function(param) {
			...
			if (selectMenu[0]) {
				...
				$.Cmdbuild.widgets.refreshCqlField(param.form, this.param.backend.widgets);
			}
		};
		...
		this.getClientDescription = function(param, callback, callbackScope) {
			var attributes = this.param.backend.getAttributes();
		...
		this.getServerDescription = function(param, callback, callbackScope) {
			if (! this.param.backend.getOriginalAttributes) {
				console.log("Backend without original attributes on form: " + param.form, this.param.backend);
				callback.apply(callbackScope, [""]);
				return;
			}
			var attributes = this.param.backend.getOriginalAttributes();
		...
		this.show = function() {
			$.Cmdbuild.utilities.proxy.getProcesses(this.showForm, this);
		};
		this.showEmptyForm = function() {
			this.param.backend["data"] = {};
			for (var i = 0; i < this.param.backend.attributes.length; i++) {
				var name = this.param.backend.attributes[i].name;
				this.param.backend.data[name] = "";
			}
			$.Cmdbuild.dataModel.push({
				form : this.param.form,
				type : "form",
				data : this.param.backend.data
			});
			this.showForm();
		};
		this.showForm = function(response) {
			// attributes
			this.initializationPhase = true;
			$.Cmdbuild.dataModel.evaluateXmlAttributes(this.param.form, this.param.backend.getAttributes());
			this.noGroupDefinition(this.param.backend.getAttributes());
			$.Cmdbuild.CqlManager.compile(this.param.form, this.param.backend.getAttributes());
			// generate empty data
			if (this.param.emptyForm && this.param.emptyForm == "true") {
				...
				this.param.backend.data = newData;
			}
			$.Cmdbuild.dataModel.processes = response;
			try {
				$.Cmdbuild.dataModel.push({
					form : this.param.form,
					type : "form",
				...
				var htmlContainer = $("#" + this.param.container)[0];
				if (this.param.backend.getAttributes().length == 0) {
				...
				var widgets = this.param.backend.widgets;
