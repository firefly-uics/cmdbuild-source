...
function Configurator(output) {
	...
	this.manager = {
			...
			getValue : function(formName, name, callback, callbackScope) {
				try {
					...
					if (ar.length > 1 && ar[1].toUpperCase() == "DESCRIPTION" ) {
						...
						var value = $.Cmdbuild.dataModel.getValue(formName, ar[0]);
						callback.apply(callbackScope, [value || undefined]);
					}
				}
				catch(e) {
					console.log("Error on server variable "+ formName + " " + name + " " + e.message);
		...
		user : {
			isVariableManager : true,
			getValue : function(form, name, callback, callbackScope) {
				callback.apply(callbackScope, [window.userId]);
			}
		},
		cql : {
			...
			getValue : function(form, filter, callback, callbackScope) {
				...
				$.Cmdbuild.utilities.proxy.getCqlResult(config, function(response) {
					callback.apply(callbackScope, [response]);
				}, this);
			}
		},
			...
			safeJSEval : function(stringTOEvaluate) {
				...
				try {
					...
					try {
						...
						output({
							type : "debug",
							msg : "Error evaluating javascript expression " + stringTOEvaluate
						});
					}
				}
				output({
	...
	this.changed = function(form, field) {
		...
		if ($.Cmdbuild.custom.commands && $.Cmdbuild.custom.commands.fieldRefresh) {
