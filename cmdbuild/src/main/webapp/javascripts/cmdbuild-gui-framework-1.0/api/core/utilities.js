...
*/				callback.apply(callbackScope, [callbackParams]);
			...
			var fullUrlFile = "";
			if (fileType && fileType === "widget") {
				fullUrlFile = $.Cmdbuild.global.getAppRootUrl() + "widgets/"+ file;//file;
			}
			else {
		...
		getHtmlFieldValue: function(name) {
			...
			if (field) {
				if (field.prop("tagName") == "SELECT") {
					var entryValue = $(name + " :selected").attr("entryValue");
					return entryValue;
				}
		...
		convertDateGUI2DB: function(entryValue) {
		...
		convertDateDB2GUI: function(entryValue) {
		...
		startApplication: function() {
			if ($.Cmdbuild.custom.commands && $.Cmdbuild.custom.commands.initialize) {
				$.Cmdbuild.custom.commands.initialize(this.startApplicationCallBack);
			}
			else {
				this.startApplicationCallBack();
			}
		},
