(function () {
	...
	Ext.define('CMDBuild.controller.management.widget.linkCards.LinkCardsController', {
		...
		requires: [
			...
			'CMDBuild.core.Utils',
			'CMDBuild.proxy.management.widget.LinkCards'
		],
		...
		/**
		 * @property {CMDBuild.view.management.widget.linkCards.LinkCardsGrid}
		 */
		grid: undefined,

		/**
		 * @property {CMDBuild.controller.management.widget.linkCards.LinkCardsMapController}
		 */
		...
		/**
		 * @param {CMDBuild.view.management.widget.linkCards.LinkCards} view
		...
		constructor: function (view, ownerController, widgetConf, clientForm, card) {
			...
			if (this.gisMapEnabled && !Ext.isEmpty(this.view.getMapPanel()))
				this.mapController = Ext.create('CMDBuild.controller.management.widget.linkCards.LinkCardsMapController', {
		...
		getCardWindow: function (model, editable) {
			var cardWindow = Ext.create('CMDBuild.view.management.widget.linkCards.cardWindow.CMCardWindow', {
				cmEditMode: editable,
				withButtons: editable,
				title: model.get('IdClass_value')
			});

			Ext.create('CMDBuild.controller.management.widget.linkCards.cardWindow.CMCardWindowController', {
				view: cardWindow,
				configuration: {
		...
		onGridShow: function () {
			...
			if (!Ext.isEmpty(lastSelectionId)) {
				...
				CMDBuild.proxy.management.widget.LinkCards.readCardPosition({
					params: params,
					loadMask: false,
					scope: this,
			...
			this.templateResolver.resolveTemplates({
				...
				callback: function (out, ctx) {
					...
					if (Ext.isObject(defaultSelection) && !Ext.Object.isEmpty(defaultSelection)) {
						...
						defaultSelection[CMDBuild.core.constants.Proxy.CLASS_NAME] = this.targetClass.get(CMDBuild.core.constants.Proxy.NAME);
