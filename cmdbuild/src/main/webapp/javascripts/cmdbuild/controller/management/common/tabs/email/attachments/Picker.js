(function () {
	...
	Ext.define('CMDBuild.controller.management.common.tabs.email.attachments.Picker', {
		...
		onPickerWindowCardSelected: function(record) {
			...
			var params = {};
			params[CMDBuild.core.proxy.Constants.CARD_ID] = this.selectedCard.get(CMDBuild.core.proxy.Constants.ID);
			params[CMDBuild.core.proxy.Constants.CLASS_NAME] = _CMCache.getEntryTypeNameById(this.selectedCard.get('IdClass'));
		...
		onPickerWindowClassSelected: function() {
			...
			this.view.cardGrid.updateStoreForClassId(this.selectedClass.get(CMDBuild.core.proxy.Constants.ID));
		},
		...
		onPickerWindowConfirmButtonClick: function() {
			if (this.attachmentGridSelectionModel.hasSelection()) {
				...
				Ext.Array.forEach(this.attachmentGridSelectionModel.getSelection(), function(attachment, i, allAttachments) {
					...
					params[CMDBuild.core.proxy.Constants.CLASS_NAME] = this.selectedClass.get(CMDBuild.core.proxy.Constants.NAME);
					params[CMDBuild.core.proxy.Constants.CARD_ID] = this.selectedCard.get(CMDBuild.core.proxy.Constants.ID);
					params[CMDBuild.core.proxy.Constants.FILE_NAME] = attachment.get('Filename');
