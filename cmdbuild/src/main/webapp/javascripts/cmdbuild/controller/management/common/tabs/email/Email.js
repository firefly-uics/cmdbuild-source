(function () {
	...
	Ext.define('CMDBuild.controller.management.common.tabs.email.Email', {
		...
		statics: {
			...
			trafficLightArrayCheck: function(record, regenerationTrafficLightArray) {
				if (!Ext.isEmpty(regenerationTrafficLightArray) && Ext.isArray(regenerationTrafficLightArray)) {
					...
					Ext.Array.forEach(regenerationTrafficLightArray, function(item, index, allItems) {
						if (Ext.Object.equals(item[CMDBuild.core.proxy.Constants.RECORD], record))
							item[CMDBuild.core.proxy.Constants.STATUS] = true;

						if (!item[CMDBuild.core.proxy.Constants.STATUS])
			...
			trafficLightSlotBuild: function(record, trafficLightArray) {
				if (!Ext.isEmpty(record) && Ext.isArray(trafficLightArray)) {
					var trafficLight = {};
					trafficLight[CMDBuild.core.proxy.Constants.STATUS] = false;
					trafficLight[CMDBuild.core.proxy.Constants.RECORD] = record; // Reference to record
		...
		bindLocalDepsChangeEvent: function(record, templateResolver, scope) {
			templateResolver.bindLocalDepsChange(function() {
				if (
					...
					if (!record.get(CMDBuild.core.proxy.Constants.KEEP_SYNCHRONIZATION) && !record.get(CMDBuild.core.proxy.Constants.PROMPT_SYNCHRONIZATION))
		...
		checkCondition: function(data, templateResolver) {
			var conditionExpr = data[CMDBuild.core.proxy.Constants.CONDITION];
		...
		checkTemplatesToRegenerate: function() {
			...
			Ext.Array.forEach(this.emailTemplatesObjects, function(template, templateIndex, allTemplatesItems) {
				if (!Ext.Object.isEmpty(template))
					var mergedTemplate = Ext.apply(template.getData(), template.get(CMDBuild.core.proxy.Constants.VARIABLES));
					...
					Ext.Object.each(mergedTemplate, function(key, value, myself) {
						if (Ext.isString(value)) { // Check all types of CQL variables that can contains client variables
							CMDBuild.controller.management.common.tabs.email.Email.searchForCqlClientVariables(
								value,
								mergedTemplate[CMDBuild.core.proxy.Constants.KEY] || mergedTemplate[CMDBuild.core.proxy.Constants.NAME],
								dirtyVariables,
		...
		extractVariablesForTemplateResolver: function() {
			...
			Ext.Array.forEach(this.emailTemplatesObjects, function(item, index, allItems) {
				...
				var templateVariables = item.get(CMDBuild.core.proxy.Constants.VARIABLES);
		...
		getAllTemplatesData: function() {
			...
			Ext.Array.forEach(this.configurationTemplates, function(template, index, allItems) {
				if (!Ext.isEmpty(template) && !Ext.Array.contains(this.emailTemplatesIdentifiers, template.get(CMDBuild.core.proxy.Constants.KEY))) {
					this.emailTemplatesObjects.push(template);
					this.emailTemplatesIdentifiers.push(template.get(CMDBuild.core.proxy.Constants.KEY));
				}
			}, this);
			...
			Ext.Array.forEach(this.controllerGrid.getDraftEmails(), function(record, index, allItems) {
				...
				var template = record.get(CMDBuild.core.proxy.Constants.TEMPLATE);

				if (Ext.isObject(template)) {
					templateIdentifier = template.get(CMDBuild.core.proxy.Constants.KEY) || template.get(CMDBuild.core.proxy.Constants.NAME);
		...
		onEmailPanelShow: function() {
			this.view.setDisabled(
				Ext.isEmpty(this.selectedEntity.get(CMDBuild.core.proxy.Constants.ENTITY))
				...
				if (this.regenerateAllEmailsGet()) {
					...
					if (this.forceRegenerationGet() || this.relatedAttributeChanged) {
						...
						Ext.Array.forEach(this.controllerGrid.getDraftEmails(), function(item, i, allItems) {
							var recordTemplate = item.get(CMDBuild.core.proxy.Constants.TEMPLATE);
							...
							if (
								...
								&& item.get(CMDBuild.core.proxy.Constants.KEEP_SYNCHRONIZATION)
							) {
								if (item.get(CMDBuild.core.proxy.Constants.PROMPT_SYNCHRONIZATION) && !this.forceRegenerationGet()) { // PromptSynch implementation
						...
						Ext.Array.forEach(this.configurationTemplates, function(item, i, allItems) {
							var templateIdentifier = item.get(CMDBuild.core.proxy.Constants.KEY);
		...
		regenerateEmail: function(record, regenerationTrafficLightArray) {
			if (
				...
				&& !Ext.isEmpty(record.get(CMDBuild.core.proxy.Constants.TEMPLATE))
				&& record.get(CMDBuild.core.proxy.Constants.KEEP_SYNCHRONIZATION)
			) {
				var me = this;

				// Find record template in emailTemplatesObjects
				var recordTemplate = record.get(CMDBuild.core.proxy.Constants.TEMPLATE);
				recordTemplate = Ext.Array.findBy(this.emailTemplatesObjects, function(item, index) {
					if (
						recordTemplate == item.get(CMDBuild.core.proxy.Constants.KEY)
						|| recordTemplate == item.get(CMDBuild.core.proxy.Constants.NAME)
					) {
						return true;
					}
				...
				if (!Ext.isEmpty(recordTemplate)) {
					var templateData = Ext.apply({}, recordTemplate.getData(), recordTemplate.get(CMDBuild.core.proxy.Constants.VARIABLES));
					...
					var templateResolver = new CMDBuild.Management.TemplateResolver({
						...
						serverVars: CMDBuild.controller.management.common.widgets.CMWidgetController.getTemplateResolverServerVars(
							this.selectedEntity.get(CMDBuild.core.proxy.Constants.ENTITY)
						)
					});
					...
					templateResolver.resolveTemplates({
						...
						callback: function(values, ctx) {
							...
							if (me.checkCondition(values, templateResolver)) {
								_msg('Email with subject "' + values[CMDBuild.core.proxy.Constants.SUBJECT] + '" regenerated');
		...
		regenerateSelectedEmails: function(records) {
			if (!Ext.isEmpty(records)) {
				...
				Ext.Array.forEach(records, function(item, i, allItems) {
					...
					if (!Ext.isEmpty(recordTemplate)) {
						if (Ext.isEmpty(item.get(CMDBuild.core.proxy.Constants.ID))) { // If there isn't an id the record is a new email generated from template
							this.regenerateTemplate(item, regenerationTrafficLightArray);
						} else {
							this.regenerateEmail(item, regenerationTrafficLightArray);
						}
		...
		regenerateTemplate: function(template, regenerationTrafficLightArray) {
			if (
				...
				var me = this;
				var xaVars = Ext.apply({}, template.getData(), template.get(CMDBuild.core.proxy.Constants.VARIABLES));
				...
				templateResolver.resolveTemplates({
					...
					callback: function(values, ctx) {
						...
						var record = Ext.Array.findBy(me.controllerGrid.getDraftEmails(), function(item, index) {
							if (item.get(CMDBuild.core.proxy.Constants.TEMPLATE) == template.get(CMDBuild.core.proxy.Constants.KEY))
								return true;
						...
						emailObject.set(CMDBuild.core.proxy.Constants.REFERENCE, me.cmfg('selectedEntityIdGet'));
						emailObject.set(CMDBuild.core.proxy.Constants.TEMPLATE, template.get(CMDBuild.core.proxy.Constants.KEY));
						emailObject.set(CMDBuild.core.proxy.Constants.TEMPORARY, me.cmfg('selectedEntityIdGet') < 0); // Setup temporary parameter

						if (me.checkCondition(values, templateResolver)) {
							_msg('Template with subject "' + values[CMDBuild.core.proxy.Constants.SUBJECT] + '" regenerated');
		...
		// SelectedEntity property functions
			...
			selectedEntityIdGet: function() {
				...
				return this.selectedEntity.get(CMDBuild.core.proxy.Constants.ID);
			},
			...
			selectedEntitySet: function(selectedEntity, callbackFunction) {
				...
				if (Ext.isEmpty(selectedEntity)) {
					var params = {};
					params[CMDBuild.core.proxy.Constants.NOT_POSITIVES] = true;
				...
				} else if (Ext.isEmpty(selectedEntity.get(CMDBuild.core.proxy.Constants.ID))) {
					var params = {};
					params[CMDBuild.core.proxy.Constants.NOT_POSITIVES] = true;
					...
					this.selectedEntity = Ext.create('CMDBuild.model.common.tabs.email.SelectedEntity', {
						id: selectedEntity.get(CMDBuild.core.proxy.Constants.ID),
						entity: selectedEntity
					});
