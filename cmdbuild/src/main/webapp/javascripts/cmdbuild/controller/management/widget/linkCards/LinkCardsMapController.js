(function () {
	...
	Ext.define('CMDBuild.controller.management.widget.linkCards.LinkCardsMapController', {
		...
		/**
		 * @property {CMDBuild.controller.management.widget.linkCards.map.CMMapEditingWindowDelegate}
		 */
		...
		/**
		 * @property {CMDBuild.controller.management.widget.linkCards.map.CMMiniCardGridWindowFeaturesController}
		 */
		...
		/**
		 * @property {CMDBuild.controller.management.widget.linkCards.map.CMSelectFeatureController}
		 */
		...
		/**
		 * @property {CMDBuild.view.management.widget.linkCards.map.CMMapPanel}
		 */
		view: undefined,
			...
			this.mon(
				...
				function () {
					...
					this.view.addDelegate(
						...
						new CMDBuild.controller.management.widget.linkCards.map.CMMapLayerSwitcherController(layerSwitcher, this.map)
					);
					...
					if (cardBrowserPanel) {
						// TODO: use Ext.create with object as parameter
						new CMDBuild.controller.management.widget.linkCards.map.CMCardBrowserTreeDataSource(cardBrowserPanel, this.mapState);
						cardBrowserPanel.addDelegate(Ext.create('CMDBuild.controller.management.widget.linkCards.map.CMCardBrowserDelegate', this));
					}
					...
					this.miniCardGridWindowController = Ext.create('CMDBuild.controller.management.widget.linkCards.map.CMMiniCardGridWindowFeaturesController');

					// Initialize editing control
					this.editingWindowDelegate = Ext.create('CMDBuild.controller.management.widget.linkCards.map.CMMapEditingWindowDelegate', this);
					this.view.editingWindow.addDelegate(this.editingWindowDelegate);
					this.selectControl = new CMDBuild.controller.management.widget.linkCards.map.CMSelectFeatureController([], {
						hover: false,
		 ...
		 * @param {CMDBuild.controller.management.widget.linkCards.map.CMSelectFeatureController} e.object
		 */
