	...
	Ext.define('CMDBuild.controller.administration.workflow.CMProcessTasksController', {
			...
			Ext.Function.createDelayed(function() {
				...
				Ext.Function.createDelayed(function() {
					...
					this.targetController.form.delegate.delegateStep[0].setValueWorkflowCombo(
						this.currentProcess.get(CMDBuild.core.proxy.Constants.NAME)
					);
		...
		onItemDoubleClick: function(name, param, callBack) {
			...
			Ext.Function.createDelayed(function() {
				...
				this.targetController.grid.getStore().load({
					scope: this,
					callback: function() {
						var selectionIndex = this.targetController.grid.getStore().find(CMDBuild.core.proxy.Constants.ID, param.id);
		...
		onProcessSelected: function(processId, process) {
			...
			if (!process || process.get('superclass')) {
				...
				this.grid.store.load({
					params: {
						workflowClassName: process.get(CMDBuild.core.proxy.Constants.NAME)
					}
				});
			}
		},
		...
		onRowSelected: function() {
			if (this.selectionModel.hasSelection()) {
				this.currentProcessTaskId = this.selectionModel.getSelection()[0].get(CMDBuild.core.proxy.Constants.ID);
