	...
	Ext.define('CMDBuild.controller.administration.tasks.CMTasksFormConnectorController', {
		...
		getStoreFilteredClass: function() {
			...
			for (var item in selectedClassArray) {
				...
				bufferObj[CMDBuild.core.proxy.Constants.NAME] = selectedClassArray[item];
		...
		getStoreFilteredSource: function() {
			...
			for (var item in selectedSourceArray) {
				...
				bufferObj[CMDBuild.core.proxy.Constants.NAME] = selectedSourceArray[item];
				...
				this.selectedDataStore.load({
					...
					callback: function(records, operation, success) {
						if (!Ext.isEmpty(records)) {
							...
							this.delegateStep[0].setValueNotificationAccount(record.get(CMDBuild.core.proxy.Constants.NOTIFICATION_EMAIL_ACCOUNT));
							this.delegateStep[0].setValueNotificationFieldsetCheckbox(record.get(CMDBuild.core.proxy.Constants.NOTIFICATION_ACTIVE));
							this.delegateStep[0].setValueNotificationTemplateError(record.get(CMDBuild.core.proxy.Constants.NOTIFICATION_EMAIL_TEMPLATE_ERROR));

							// Set step2 [1] datas
							this.delegateStep[1].setValueAdvancedFields(record.get(CMDBuild.core.proxy.Constants.CRON_EXPRESSION));
							this.delegateStep[1].setValueBase(record.get(CMDBuild.core.proxy.Constants.CRON_EXPRESSION));

							// Set step3 [2] datas
							this.delegateStep[2].setValueDataSourceConfiguration(
								record.get(CMDBuild.core.proxy.Constants.DATASOURCE_TYPE),
								record.get(CMDBuild.core.proxy.Constants.DATASOURCE_CONFIGURATION)
							);

							// Set step4 [3] datas
							this.delegateStep[3].setData(record.get(CMDBuild.core.proxy.Constants.CLASS_MAPPING));

							// Set step5 [4] datas
							this.delegateStep[4].setData(record.get(CMDBuild.core.proxy.Constants.ATTRIBUTE_MAPPING));
		...
		onSaveButtonClick: function() {
			...
			if (this.validate(formData[CMDBuild.core.proxy.Constants.ACTIVE])) {
				...
				// Fieldset submitting filter to avoid to send datas if fieldset are collapsed
					...
					if (notificationFieldsetCheckboxValue) {
						...
						submitDatas[CMDBuild.core.proxy.Constants.NOTIFICATION_EMAIL_TEMPLATE_ERROR] = formData[CMDBuild.core.proxy.Constants.NOTIFICATION_EMAIL_TEMPLATE_ERROR];
					}

				submitDatas[CMDBuild.core.proxy.Constants.CRON_EXPRESSION] = this.delegateStep[1].getCronDelegate().getValue();
				...
				// Form submit values formatting
					...
					if (dataSourceType) {
						...
						switch (dataSourceType) {
							case 'db': {
								configurationObject[CMDBuild.core.proxy.Constants.DATASOURCE_ADDRESS] = formData[CMDBuild.core.proxy.Constants.DATASOURCE_ADDRESS];
								configurationObject[CMDBuild.core.proxy.Constants.DATASOURCE_DB_NAME] = formData[CMDBuild.core.proxy.Constants.DATASOURCE_DB_NAME];
								configurationObject[CMDBuild.core.proxy.Constants.DATASOURCE_DB_PASSWORD] = formData[CMDBuild.core.proxy.Constants.DATASOURCE_DB_PASSWORD];
								configurationObject[CMDBuild.core.proxy.Constants.DATASOURCE_DB_PORT] = formData[CMDBuild.core.proxy.Constants.DATASOURCE_DB_PORT];
								configurationObject[CMDBuild.core.proxy.Constants.DATASOURCE_DB_TYPE] = formData[CMDBuild.core.proxy.Constants.DATASOURCE_DB_TYPE];
								configurationObject[CMDBuild.core.proxy.Constants.DATASOURCE_DB_USERNAME] = formData[CMDBuild.core.proxy.Constants.DATASOURCE_DB_USERNAME];
								configurationObject[CMDBuild.core.proxy.Constants.DATASOURCE_TABLE_VIEW_PREFIX] = formData[CMDBuild.core.proxy.Constants.DATASOURCE_TABLE_VIEW_PREFIX];

								if (!Ext.isEmpty(formData[CMDBuild.core.proxy.Constants.DATASOURCE_DB_INSATANCE_NAME]))
									configurationObject[CMDBuild.core.proxy.Constants.DATASOURCE_DB_INSATANCE_NAME] = formData[CMDBuild.core.proxy.Constants.DATASOURCE_DB_INSATANCE_NAME];
							} break;
						...
						submitDatas[CMDBuild.core.proxy.Constants.DATASOURCE_TYPE] = dataSourceType;
						submitDatas[CMDBuild.core.proxy.Constants.DATASOURCE_CONFIGURATION] = Ext.encode(configurationObject);
					}

					var classMappingData = this.delegateStep[3].getData();
					if (!Ext.isEmpty(classMappingData))
						submitDatas[CMDBuild.core.proxy.Constants.CLASS_MAPPING] = Ext.encode(classMappingData);

					var attributeMappingData = this.delegateStep[4].getData();
					if (!Ext.isEmpty(attributeMappingData))
						submitDatas[CMDBuild.core.proxy.Constants.ATTRIBUTE_MAPPING] = Ext.encode(attributeMappingData);
			...
			if (gridStore.count() > 0) {
				gridStore.each(function(record, id) {
					if (
						...
						&& !Ext.Array.contains(this.delegateStep[3].getSelectedClassArray(), record.get(CMDBuild.core.proxy.Constants.CLASS_NAME))
					)
						record.set(CMDBuild.core.proxy.Constants.CLASS_NAME, '');

					if (
						!Ext.isEmpty(record.get(CMDBuild.core.proxy.Constants.SOURCE_NAME))
						&& !Ext.Array.contains(this.delegateStep[3].getSelectedSourceArray(), record.get(CMDBuild.core.proxy.Constants.SOURCE_NAME))
					)
						record.set(CMDBuild.core.proxy.Constants.SOURCE_NAME, '');
				}, this);
			}
		}
	});
