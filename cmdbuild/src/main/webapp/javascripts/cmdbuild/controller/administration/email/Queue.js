	...
	Ext.define('CMDBuild.controller.administration.email.Queue', {
		...
		onEmailQueueSaveButtonClick: function() {
			if (this.validate(this.view)) { // Validate before save
				var params = {};
				params[CMDBuild.core.proxy.Constants.TIME] = this.toManagedUnit(this.view.cycleIntervalField.getValue());
		...
		readConfiguration: function() {
			CMDBuild.core.proxy.email.Queue.configurationRead({
				...
				success: function(response, options, decodedResponse) {
					...
					this.view.cycleIntervalField.setValue(this.toDisplayedUnit(configurationModel.get(CMDBuild.core.proxy.Constants.TIME)));
				},
