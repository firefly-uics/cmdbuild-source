	...
	Ext.define('CMDBuild.controller.administration.tasks.CMTasksFormEmailController', {
		...
		removeItem: function() {
			if (!Ext.isEmpty(this.selectedId)) {
				...
				CMDBuild.proxy.administration.taskManager.Email.remove({
					params: {
						id: this.selectedId
					},
					loadMask: false,
		...
		onRowSelected: function() {
			if (this.selectionModel.hasSelection()) {
				...
				CMDBuild.proxy.administration.taskManager.Email.read({
					params: {
						id: this.selectedId
					},
					loadMask: false,
		...
		onSaveButtonClick: function() {
			...
			if (this.validate(formData[CMDBuild.core.constants.Proxy.ACTIVE])) {
				...
				if (Ext.isEmpty(formData[CMDBuild.core.constants.Proxy.ID])) {
					CMDBuild.proxy.administration.taskManager.Email.create({
						params: submitDatas,
						loadMask: false,
						scope: this,
						success: this.success,
					...
					CMDBuild.proxy.administration.taskManager.Email.update({
						params: submitDatas,
						loadMask: false,
						scope: this,
						success: this.success,
