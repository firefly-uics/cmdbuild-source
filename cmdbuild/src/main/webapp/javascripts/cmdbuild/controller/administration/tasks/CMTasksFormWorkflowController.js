	...
	Ext.define('CMDBuild.controller.administration.tasks.CMTasksFormWorkflowController', {
		...
		removeItem: function() {
			if (!Ext.isEmpty(this.selectedId)) {
				...
				CMDBuild.proxy.administration.taskManager.Workflow.remove({
					params: {
						id: this.selectedId
					},
					loadMask: false,
		...
		onRowSelected: function() {
			if (this.selectionModel.hasSelection()) {
				...
				CMDBuild.proxy.administration.taskManager.Workflow.read({
					params: {
						id: this.selectedId
					},
					loadMask: false,
		...
		onSaveButtonClick: function() {
			...
			if (this.validate(formData[CMDBuild.core.constants.Proxy.ACTIVE])) {
				...
				if (Ext.isEmpty(formData[CMDBuild.core.constants.Proxy.ID])) {
					CMDBuild.proxy.administration.taskManager.Workflow.create({
						params: submitDatas,
						loadMask: false,
						scope: this,
						success: this.success,
					...
					CMDBuild.proxy.administration.taskManager.Workflow.update({
						params: submitDatas,
						loadMask: false,
						scope: this,
						success: this.success,
