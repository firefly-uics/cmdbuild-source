	...
	Ext.define('CMDBuild.controller.administration.tasks.CMTasksFormEventController', {
		...
		removeItem: function() {
			if (!Ext.isEmpty(this.selectedId)) {
				...
				CMDBuild.core.proxy.taskManager.Event.remove({ // TODO
					params: {
						id: this.selectedId
					},
					loadMask: false,
		...
		onRowSelected: function() {
			if (this.selectionModel.hasSelection()) {
				...
				switch (this.selectedType) {
					case 'event_asynchronous': {
						CMDBuild.core.proxy.taskManager.event.Asynchronous.read({
							params: {
								id: this.selectedId
							},
							loadMask: false,
...
//									this.delegateStep[3].setValueWorkflowFieldsetCheckbox(record.get(CMDBuild.core.constants.Proxy.WORKFLOW_ACTIVE));
					...
					case 'event_synchronous': {
						CMDBuild.core.proxy.taskManager.event.Synchronous.read({
							params: {
								id: this.selectedId
							},
							loadMask: false,
...
//								}
				...
				if (Ext.isEmpty(formData[CMDBuild.core.constants.Proxy.ID])) {
					switch (taskType) {
						case 'event_asynchronous': {
							CMDBuild.core.proxy.taskManager.event.Asynchronous.create({
								params: submitDatas,
								loadMask: false,
								scope: this,
								success: this.success,
						...
						case 'event_synchronous': {
							CMDBuild.core.proxy.taskManager.event.Synchronous.create({
								params: submitDatas,
								loadMask: false,
								scope: this,
								success: this.success,
					...
					switch (taskType) {
						case 'event_asynchronous': {
							CMDBuild.core.proxy.taskManager.event.Asynchronous.update({
								params: submitDatas,
								loadMask: false,
								scope: this,
								success: this.success,
						...
						case 'event_synchronous': {
							CMDBuild.core.proxy.taskManager.event.Synchronous.update({
								params: submitDatas,
								loadMask: false,
								scope: this,
								success: this.success,
		...
		removeItem: function() {
			if (!Ext.isEmpty(this.selectedId)) {
				...
				switch (this.selectedType) {
					case 'event_asynchronous': {
						CMDBuild.core.proxy.taskManager.event.Asynchronous.remove({
							params: {
								id: this.selectedId
							},
							loadMask: false,
					...
					case 'event_synchronous': {
						CMDBuild.core.proxy.taskManager.event.Synchronous.remove({
							params: {
								id: this.selectedId
							},
							loadMask: false,
