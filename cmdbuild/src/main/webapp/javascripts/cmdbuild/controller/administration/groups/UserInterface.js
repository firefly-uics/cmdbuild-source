(function() {
	...
	Ext.define('CMDBuild.controller.administration.groups.UserInterface', {
		extend: 'CMDBuild.controller.common.AbstractController',

		requires: [
			'CMDBuild.core.Message',
			'CMDBuild.core.proxy.CMProxyConstants',
			'CMDBuild.core.proxy.groups.UserInterface'
		],
		...
		/**
		 * @property {CMDBuild.model.groups.userInterface.UserInterface}
		 *
		 * @private
		 */
		...
		constructor: function(configurationObject) {
			...
			this.view = Ext.create('CMDBuild.view.administration.groups.userInterface.UserInterfaceView', { delegate: this });
		...
		// Configuration property methods
			 ...
			 *
			 * @returns {CMDBuild.model.groups.userInterface.UserInterface} or Mixed
			 */
			...
			configurationSet: function(configurationObject) {
				...
				if (!Ext.isEmpty(configurationObject) && Ext.isObject(configurationObject)) {
					if (Ext.getClassName(configurationObject) == 'CMDBuild.model.groups.userInterface.UserInterface') {
						this.configuration = configurationObject;
					} else {
						this.configuration = Ext.create('CMDBuild.model.groups.userInterface.UserInterface', configurationObject);
					}
				}
			},
		...
		onGroupUserInterfaceSaveButtonClick: function() {
			...
			CMDBuild.core.proxy.groups.UserInterface.update({
				params: params,
				scope: this,
		...
		onGroupUserInterfaceTabShow: function() {
			if (!this.cmfg('selectedGroupIsEmpty')) {
				...
				CMDBuild.core.proxy.groups.UserInterface.read({
					params: params,
					scope: this,
					success: function(response, options, decodedResponse) {
