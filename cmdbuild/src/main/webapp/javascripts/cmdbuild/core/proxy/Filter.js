(function () {
	...
	Ext.define('CMDBuild.core.proxy.Filter', {
		...
		create: function (parameters) {
			...
			Ext.apply(parameters, { url: CMDBuild.core.proxy.index.Json.filter.group.create });
		...
		read: function (parameters) {
			...
			Ext.apply(parameters, { url: CMDBuild.core.proxy.index.Json.filter.group.read });
		...
		readAll: function (parameters) {
			...
			Ext.apply(parameters, { url: CMDBuild.core.proxy.index.Json.filter.groupStore });
		...
		remove: function (parameters) {
			...
			Ext.apply(parameters, { url: CMDBuild.core.proxy.index.Json.filter.group.remove });
		...
		newUserStore: function () {
			return CMDBuild.global.Cache.requestAsStore(CMDBuild.core.constants.Proxy.UNCACHED, {
				...
				proxy: {
					type: 'ajax',
					url: CMDBuild.core.proxy.index.Json.filter.user.readAll,
					reader: {
						type: 'json',
		...
		newSystemStore: function (className) {
			return CMDBuild.global.Cache.requestAsStore(CMDBuild.core.constants.Proxy.UNCACHED, {
				...
				pageSize: CMDBuild.configuration.instance.get(CMDBuild.core.constants.Proxy.ROW_LIMIT),
				proxy: {
					url: CMDBuild.core.proxy.index.Json.filter.group.read,
					type: 'ajax',
					reader: {
						type: 'json',
						root: CMDBuild.core.constants.Proxy.FILTERS,
