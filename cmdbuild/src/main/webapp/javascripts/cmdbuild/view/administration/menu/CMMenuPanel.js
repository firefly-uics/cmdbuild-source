(function() {
	...
	Ext.define('CMDBuild.Administration.MenuPanel', {
		...
		initComponent: function() {
			...
			if (true) // TODO: To enable only if translation edit button must be displayed
				treeColumns.push({
					xtype: 'actioncolumn',
					width: 30,
					align: 'center',
			...
			this.treePanel = Ext.create('Ext.tree.Panel', {
				...
				plugins: [
					Ext.create('Ext.grid.plugin.CellEditing', {
						...
						listeners: {
							beforeedit: function(editor, e, eOpts) {_debug(e.row.lastElementChild.children);
								// show the icon
								var cmp = Ext.select('#' + e.row.id + ' [class~="x-action-col-icon"]');
