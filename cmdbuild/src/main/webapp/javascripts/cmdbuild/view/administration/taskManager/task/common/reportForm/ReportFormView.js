(function () {
	...
	Ext.define('CMDBuild.view.administration.taskManager.task.common.reportForm.ReportFormView', {
		...
		/**
		 * @property {CMDBuild.view.administration.taskManager.task.common.reportForm.GridPanel}
		 */
		grid: undefined,
		...
		initComponent: function () {
			this.delegate = Ext.create('CMDBuild.controller.administration.taskManager.task.common.ReportForm', { view: this });
			...
			Ext.apply(this, {
				items: [
					this.combo = Ext.create('Ext.form.field.ComboBox', {
						...
						store: CMDBuild.proxy.administration.taskManager.task.common.ReportForm.getStore(),
						queryMode: 'local',
					...
					this.extension = Ext.create('Ext.form.field.ComboBox', { // Prepared for future implementations
						...
						store: CMDBuild.proxy.administration.taskManager.task.common.ReportForm.getStoreExtension(),
						queryMode: 'local'
					}),
					this.grid = Ext.create('CMDBuild.view.administration.taskManager.task.common.reportForm.GridPanel', { delegate: this.delegate })
				]
			});
