(function() {
	...
	Ext.define('CMCardBrowserNodeModel', {
		extend: 'Ext.data.Model',
		idProperty: "cardId",
		fields: [
			{ name: 'text', type: 'string' },
			{ name: 'visible', type: 'boolean'},
			{ name: 'cardId', type: 'string' }, // cardId or the name of a geoserver layer
			{ name: 'className', type: 'string' },
			{ name: 'classId', type: 'int' },
			{ name: 'baseNode', type: 'boolean' } // To identify the exclusive nodes that represent the base for the vertical overlap
		],
	...
	Ext.define("CMDBuild.view.management.widget.linkCards.map.CMCardBrowserTreeDelegate", {
		/**
		 *
		 * @param {CMDBuild.view.management.widget.linkCards.map.CMCardBrowserTree} tree The tree who call the method
		...
		/**
		 * @param {CMDBuild.view.management.widget.linkCards.map.CMCardBrowserTree} tree The activated panel
	...
	Ext.define("CMDBuild.view.management.widget.linkCards.map.CMCardBrowserTree", {
		...
		constructor: function(ds) {
			this.mixins.delegable.constructor.call(this, "CMDBuild.view.management.widget.linkCards.map.CMCardBrowserTreeDelegate");
		...
		initComponent: function() {
			...
			this.columns = [{
				...
				handler: function(grid, rowIndex, colIndex, actionItem, event, record, row) {
					...
					this.icon = value ? 'images/icons/bullet_go.png' : 'images/icons/cancel.png';
					record.set("visible", !value);
					record.commit();

					var deeply = false;
					_debug("SINGLE VISIBILITY", value);
					me.callDelegates("onCardBrowserTreeCheckChange", [
							me, record, value, deeply
					]);
				}
			}, {
				width: 40,
		...
		selectNodeSilently: function(node) {
			...
			try {
				...
				_error("ERROR selecting the CardBrowserTree", this, e);
			}
		},
	...
	function deselectAllSilently(me) {
		try {
			...
			_error("ERROR deselecting the CardBrowserTree", this, e);
		}
	}
