(function() {
	 ...
	 *                params.object The OpenLayer map which has added the
	 *                layer
	 ...
	 *                params.layer The OpenLayer layer which is removed
	 * @param {OpenLayers.Layer}
	 *                params.property The layer property that is changed,
	 *                one of [name, order, opacity, params, visibility or
	 *                attribution]
	 */
    ...
    Ext
	    .define(
		    "CMDBuild.view.management.classes.map.CMMapPanel",
		    {
			...
			requires : [
				'CMDBuild.controller.management.classes.map.CM16CardGrid',
			...
			initComponent : function() {
			    var me = this;
			    this.geoExtension = Ext
				    .create('CMDBuild.view.management.classes.map.geoextension.CMDBuildGeoExt');
			    var thematicDocument = Ext
				    .create('CMDBuild.view.management.classes.map.thematism.ThematicDocument');
			    var strategiesManager = Ext
				    .create('CMDBuild.view.management.classes.map.thematism.ThematicStrategiesManager');
			    thematicDocument
				    .configureStrategiesManager(strategiesManager);
			    thematicDocument
				    .setThematismButton(this.thematismButton);
			    this.interactionDocument = Ext
				    .create('CMDBuild.view.management.classes.map.geoextension.InteractionDocument');
			    var thematicColors = Ext
				    .create('CMDBuild.view.management.classes.map.thematism.ThematicColors');
			    thematicDocument.init(this.interactionDocument,
				    thematicColors)
			    this.interactionDocument
				    .setThematicDocument(thematicDocument);
			    this.mapPanel = Ext
				    .create(
				...
				// proxy
				// constants
				var root = CMDBuild.configuration.gis
					.get('cardBrowserByDomainConfiguration')['root']; // TODO:
				// use
				// proxy
				// constants
				...
				this.cardBrowser = new CMDBuild.view.management.CMCardBrowserTree(
					{
					    ...
					    frame : false,
					    border : false,
					    rootText : root.classDescription
						    || root.className
					});

				tabs.push(this.cardBrowser);
				this.thematicView = Ext
					.create(
						'CMDBuild.controller.management.classes.map.thematism.ThematismMainWindow',
						{
						    interactionDocument : this.interactionDocument
			...
			setCmVisible : function(visible) {
			    ...
			    this.callDelegates("onMapPanelVisibilityChanged", [
				    this, visible ]);
			},
