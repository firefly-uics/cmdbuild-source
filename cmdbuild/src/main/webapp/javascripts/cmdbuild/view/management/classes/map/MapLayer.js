(function() {
	...
	/*
	 * This method is called after a request, there are problems when remove
	 * the layer from the map... it seems that the strategy does not realize
	 * that the layer is now null
	 * 
	 * (...and I deactivate the strategy before remove the layer...)
	 */
	merge : function(resp) {
	    if (this.layer) {
		return OpenLayers.Strategy.BBOX.prototype.merge.apply(this,
			arguments);
	    }
	}
    });

    CMDBuild.Management.CMMap.MapLayer = OpenLayers
	    .Class(
			...
			initialize : function(name, options) {
				    ...
				    this.cmdb_minZoom = options.geoAttribute.minZoom
					    || DEFAULT_MIN_ZOOM;
			    this.cmdb_maxZoom = options.geoAttribute.maxZoom
				    || DEFAULT_MAX_ZOOM;
			    this.cmdb_index = options.geoAttribute.index;

			    // Set feature selected style with higher
			    // strokeWidth to improve visibility
			    // TODO: maybe would be better to put this in GIS
			    // config
			    OpenLayers.Feature.Vector.style["default"].strokeWidth = 5;

			    this.hiddenFeature = {};
			    this._defaultStyleConfiguration = Ext
				    .decode(options.geoAttribute.style);
			    ...
			    this.strategies = [ new CMDBuild.PatchedBBOX({
				autoActivate : true
			...
			clearSelection : function() {
				...
				this
					.addFeatures([ this.lastSelection
						.clone() ]);
			...
			getFeaturesInLonLat : function(lonlat) {
				...
				for (var i = 0, f = null, g = null; i < features.length; ++i) {
					...
					var distance = g.distanceTo(point)
						/ this.getResolution();
					var tollerance = this._defaultStyleConfiguration.pointRadius
						|| DEFAULT_POINT_DISTANCE_TOLLERANCE;
			...
			hideFeatureWithCardId : function(masterCardId, feature) {
			    var f = feature
				    || this
					    .getFeatureByMasterCard(masterCardId);
			    if (f) {
