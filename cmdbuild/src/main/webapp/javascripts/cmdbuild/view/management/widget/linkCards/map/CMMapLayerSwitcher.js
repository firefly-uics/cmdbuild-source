(function() {
	...
	Ext.define("CMDBuild.view.management.widget.linkCards.map.CMMapLayerSwitcherDelegate", {
		/**
		 *
	...
	Ext.define("CMDBuild.view.management.widget.linkCards.map.CMMapLayerSwitcher", {
		...
		constructor: function() {
			this.mixins.delegable.constructor.call(this, "CMDBuild.view.management.widget.linkCards.map.CMMapLayerSwitcherDelegate");
		...
		addLayerItem: function(layer) {
			if (layer.displayInLayerSwitcher) {
				...
				try {
					...
					_error("Fail to add layer", this, layer);
				}
			}
		},
	...
	function retrieveTargetFolder(layer, root) {
		...
		if (layer.isBaseLayer) {
			targetFolder = folderNodeByName(root, "cm_external_layers_folder");
		} else if (layer.CM_geoserverLayer) {
			targetFolder = retrieveGeoserverFolder(root);
		} else {
			targetFolder = folderNodeByName(root, "cm_cmdbuild_layers_folder");
		}
	...
	function retrieveGeoserverFolder(root) {
		var extarnalServicesFolder = folderNodeByName(root, "cm_external_layers_folder");
		var geoserverFolder = folderNodeByName(extarnalServicesFolder, "cm_geoserver_layers_folder");
		...
		if (!geoserverFolder) {
			geoserverFolder = extarnalServicesFolder.appendChild({
				...
				leaf: false,
				expanded: true,
				folderName: "cm_geoserver_layers_folder",
				checked: true
			});
		}
