...
public class QueryCreatorTest {
	...
	public void systemAttributesAreAlwaysReturned() {
		...
		assertThat(
				query,
				is("SELECT " + sqlAttribute("ac", "IdClass", "oid", "_ac_ClassId") + ","
						+ sqlAttribute("ac", "Id", "_ac_Id") + "," + sqlAttribute("ac", "User", "_ac_User") + ","
						+ sqlAttribute("ac", "BeginDate", "_ac_BeginDate") + " FROM ONLY " + sqlQuoteIdent("nc")
						+ " AS " + sqlQuoteIdent("ac") + " WHERE " + sqlAttribute("ac", "Status") + "=?"));
	}
	...
	public void anyAttributeReturnsAllAttributesAndSystem() {
		...
		assertThat(query,
				is("SELECT "
						+ sqlAttribute("ac", "nc", "nc#nca")
						+ "," // WRONG!!!!!! IT OUTPUTS
								// sqlAttribute("ac","nc#nca")
	...
	public void entryTypeAttributesAreCastToOid() {
		...
		assertThat(query,
				is("SELECT "
						+ sqlAttribute("ab", "beta", "oid", "ab#beta")
						+ "," // WRONG!!!!!! IT OUTPUTS
								// sqlAttribute("ab","nb#beta")
						+ sqlAttribute("ab", "IdClass", "oid", "_ab_ClassId") + ","
						+ sqlAttribute("ab", "Id", "_ab_Id") + "," + sqlAttribute("ab", "User", "_ab_User") + ","
						+ sqlAttribute("ab", "BeginDate", "_ab_BeginDate") + " FROM ONLY " + sqlQuoteIdent("nb")
						+ " AS " + sqlQuoteIdent("ab") + " WHERE " + sqlAttribute("ab", "Status") + "=?"));
	}
	...
	public void superclassesAreQueriedWithASubquery() {
		...
		assertThat(
				...
				is("SELECT " + sqlAttribute("ac", "IdClass", "oid", "_ac_ClassId") + ","
						+ sqlAttribute("ac", "Id", "_ac_Id") + "," + sqlAttribute("ac", "User", "_ac_User") + ","
						+ sqlAttribute("ac", "BeginDate", "_ac_BeginDate") + " FROM (" + "SELECT "
						+ sqlQuoteIdent("Id") + "," + sqlQuoteIdent("IdClass") + "," + sqlQuoteIdent("User") + ","
						+ sqlQuoteIdent("BeginDate") + "," + "NULL AS " + sqlQuoteIdent("EndDate") + " FROM ONLY "
						+ sqlQuoteIdent("na") + " UNION ALL " + "SELECT " + sqlQuoteIdent("Id") + ","
						+ sqlQuoteIdent("IdClass") + "," + sqlQuoteIdent("User") + "," + sqlQuoteIdent("BeginDate")
						+ "," + "NULL AS " + sqlQuoteIdent("EndDate") + " FROM ONLY " + sqlQuoteIdent("nb")
						+ " UNION ALL " + "SELECT " + sqlQuoteIdent("Id") + "," + sqlQuoteIdent("IdClass") + ","
						+ sqlQuoteIdent("User") + "," + sqlQuoteIdent("BeginDate") + "," + "NULL AS "
						+ sqlQuoteIdent("EndDate") + " FROM ONLY " + sqlQuoteIdent("nc") + ") AS "
						+ sqlQuoteIdent("ac") + " WHERE " + sqlAttribute("ac", "Status") + "=?"));
	}
	...
	public void joinsWithMultipleDomainsUseUnions() {
		/*
		 * THIS IS PARTIALLY TRUE: CLASS UNIONS ARE USED EVEN IF THE TARGET IS
		 * ONE CLASS
		 */
		fail();
	}
	...
	public void joinsWithSuperclassesUseUnions() {
		/*
		 * THIS IS PARTIALLY TRUE: DOMAIN UNIONS ARE USED EVEN IF OVER ONE
		 * DOMAIN ONLY
		 */
		fail();
	}
	...
	private String sqlAttribute(final String tableAlias, final String attributeName, final String cast,
			final String attributeAlias) {
		return String.format("%s.%s%s%s", sqlQuoteIdent(tableAlias), sqlQuoteIdent(attributeName), cast != null ? "::"
				+ cast : "", attributeAlias != null ? " AS " + sqlQuoteIdent(attributeAlias) : "");
	}
