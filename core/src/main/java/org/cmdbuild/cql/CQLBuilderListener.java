...
/**
 * This interface defines the method that the CQLCompilerBuilder will call when parsing the CQL expression tree
 */
public interface CQLBuilderListener {
	
	/**
	 * Handles the case of an id which is a lookup operator (e.g. Lookup.parent())
	 */
	public static class LookupOperator {
		String operator; //only parent()
		String attributeName; //can be null
		public LookupOperator(String o, String a) {
			this.operator = o; this.attributeName = a;
		}
	...
	public static class FieldInputValue {
		String variableName;
		public FieldInputValue(String v){
			this.variableName = v;
		}
	...
	public static class FieldNativeSQLValue {
		String sql;
		public FieldNativeSQLValue(String q) {
			this.sql = q;
		}
	 ...
	 * auto-sensed or, in case of ambiguity, direct.
	 * <br>
	...
	/**
	 * Called when parsing a select function is called (e.g. SELECT ... , Func( Foo,Bar ) ... )
	 * @param functionName
	 * @param functionAs
	 */
	...
	/**
	 * Called before selecting from a domain (e.g. SELECT Foo::meta(A,B)objects(C,D) )
	...
	/**
	 * Called before selecting from a domain objects (e.g. SELECT Foo::objects(A,B,C) )
	 */
	...
	/**
	 * called before starting a FROM statement,
	 * e.g. FROM A or FROM HISTORY A <br>
	...
	/**
	 * Domains can be chained, so that a definition falls under the scope of the previous domain or class.
	 * When a domain definition ends, endFromDomain is called
	 ...
	 */
	void defaultWhere(); //used when there is no WHERE statement, eg. "From A": select all active A
	/**
	...
	/**
	 * Called before starting a domain, defined in the WHERE statement, 
	 * using the domain name
	 * (e.g. FROM [Domain].objects( A>10 ) )
	 * @param type
	 * @param scopeReference
	 * @param domainName
	 * @param direction
	 * @param isNot
	 */
	void startDomain( WhereType type, String scopeReference, 
			String domainName, DomainDirection direction,
			boolean isNot);
	/**
	 * Called before starting a domain, defined in the WHERE statement,
	 * using the domain id
	 * (e.g. FROM [101123].objects( A>10 ) )
	 * @param type
	 * @param scopeReference
	 * @param domainId
	 * @param direction
	 ...
	 */
	void startDomain( WhereType type, String scopeReference,
			int domainId, DomainDirection direction,
			boolean isNot);
	
	/**
	 * Called before starting a domain, using the AS name 
	 * (e.g. FROM [DomainA] AS DomA WHERE DomA.objects( A > 10 ) )
	 * @param type
	 ...
	 * @param operator
	 */
	void startSimpleField(WhereType type, boolean isNot, 
	 ...
	 * @param operator
	 */
	void startComplexField(WhereType type, boolean isNot,
			String classOrDomainNameOrRef, String[] fieldPath,
			FieldOperator operator);
	/**
	 * Every thing that contains a 'parent()' within the id, ad es. State.parent() = 'open'
	 * @param type
	 * @param isNot
	 * @param classOrDomainNameOrRef
	 * @param lookupPath
	 * @param operator
	 */
	void startLookupField(WhereType type, boolean isNot,
			String classOrDomainNameOrRef, String fieldId,
			LookupOperator[] lookupPath,
			FieldOperator operator);
	...
	//group by
	/**
	 * Called if there is no GROUP BY statement, much probably this method will simply not be implemented.
	 */
