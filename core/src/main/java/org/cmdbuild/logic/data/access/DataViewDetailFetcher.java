...
public class DataViewDetailFetcher {
	...
	public PagedElements<CMCard> fetch( //
		...
		final QuerySpecsBuilder querySpecsBuilder = _dataView.select(anyAttribute(detailClass)).from(detailClass);
	...
	private class FilterAnalizer {
		...
		private void manageFullTextQuery( //
			...
			final String fullTextQuery = readFullTextQuery(jsonFilter);
