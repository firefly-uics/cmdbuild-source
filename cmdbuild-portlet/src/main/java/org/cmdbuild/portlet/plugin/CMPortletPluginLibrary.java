...
public class CMPortletPluginLibrary extends CMPortletPlugin {
	...
	static {
		...
		for (final String pluginClassName : PortletConfiguration.getInstance().getPlugins()) {
			try {
				final Class<CMPortletPlugin> pluginClass = (Class<CMPortletPlugin>) Class
						.forName("org.cmdbuild.portlet.plugin." + pluginClassName);
