...
public class DefaultConverter implements CmisConverter {
	...
	private static final SimpleDateFormat CMDBUILD_DATETIME_PRINTING_FORMAT = new SimpleDateFormat(Constants.DATETIME_PRINTING_PATTERN);
	private static final SimpleDateFormat CMDBUILD_DATE_PRINTING_FORMAT = new SimpleDateFormat(Constants.DATE_PRINTING_PATTERN);	
	...
	public MetadataType getType(PropertyDefinition<?> propertyDefinition) {
		if (propertyDefinition.getChoices()!=null && !propertyDefinition.getChoices().isEmpty()) {
			...
			if(type == PropertyType.BOOLEAN) {
				...
				if(((PropertyDateTimeDefinition)propertyDefinition).getDateTimeResolution() == DateTimeResolution.DATE)				
