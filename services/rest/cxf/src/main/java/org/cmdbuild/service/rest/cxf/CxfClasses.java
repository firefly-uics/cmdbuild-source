...
public class CxfClasses extends CxfService implements Classes {
	...
	@Override
	public NewCardResponse createCard(final String name, final MultivaluedMap<String, String> formParam) {
	...
	@Override
	public CardResponse readCard(final String name, final Long id) {
	...
	@Override
	public void updateCard(final String name, final Long id) {
		final CMClass targetClass = userDataView().findClass(name);
		if (targetClass == null) {
			errorHandler().classNotFound(name);
		}
		// TODO check for missing id (inside logic, please)
		final Map<String, String> attributes = transformEntries(uriInfo.getQueryParameters(), FIRST_ELEMENT);
		final Card card = Card.newInstance(targetClass) //
				.withId(id) //
				.withAllAttributes(attributes) //
				.build();
	...
	@Override
	public void deleteCard(final String name, final Long id) {
		final CMClass targetClass = userDataView().findClass(name);
		if (targetClass == null) {
			errorHandler().classNotFound(name);
		}
