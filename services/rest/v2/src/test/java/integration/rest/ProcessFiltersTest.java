...
public class ProcessFiltersTest {

	@ClassRule
	public static ServerResource<ProcessFilters> server = ServerResource.newInstance(ProcessFilters.class) //
			.withPortRange(randomPort()) //
			.build();

	private static JsonSupport json = new JsonSupport();

	private ProcessFilters service;
	private HttpClient httpclient;

	@Before
	public void setUp() throws Exception {
		server.service(service = mock(ProcessFilters.class));
	...
	public void create() throws Exception {
		...
		// when
		final HttpPost request = new HttpPost(server.resource("processes/foo/filters/"));
	...
	public void readAll() throws Exception {
		...
		// when
		final HttpGet request = new HttpGet(new URIBuilder(server.resource("processes/foo/filters/")) //
	...
	public void readSingle() throws Exception {
		...
		// when
		final HttpGet request = new HttpGet(new URIBuilder(server.resource("processes/foo/filters/42/")) //
				.setParameter(LIMIT, "123") //
				.setParameter(START, "456") //
				.build());
		final HttpResponse response = httpclient.execute(request);
	...
	public void update() throws Exception {
		// when
		final HttpPut request = new HttpPut(server.resource("processes/foo/filters/42/"));
		request.setEntity(new StringEntity("{}", APPLICATION_JSON));
		final HttpResponse response = httpclient.execute(request);
	...
	public void delete() throws Exception {
		// when
		final HttpDelete request = new HttpDelete(server.resource("processes/foo/filters/42/"));
		final HttpResponse response = httpclient.execute(request);
