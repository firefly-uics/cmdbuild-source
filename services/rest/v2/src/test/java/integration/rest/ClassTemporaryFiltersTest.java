...
public class ClassTemporaryFiltersTest {

	@ClassRule
	public static ServerResource<ClassTemporaryFilters> server = ServerResource.newInstance(ClassTemporaryFilters.class) //
			.withPortRange(randomPort()) //
			.build();

	private static JsonSupport json = new JsonSupport();

	private ClassTemporaryFilters service;
	private HttpClient httpclient;

	@Before
	public void setUp() throws Exception {
		server.service(service = mock(ClassTemporaryFilters.class));
	...
	public void create() throws Exception {
		...
		// when
		final HttpPost request = new HttpPost(server.resource("classes/foo/temporary_filters/"));
	...
	public void readAll() throws Exception {
		...
		// when
		final HttpGet request = new HttpGet(new URIBuilder(server.resource("classes/foo/temporary_filters/")) //
	...
	public void readSingle() throws Exception {
		...
		// when
		final HttpGet request = new HttpGet(new URIBuilder(server.resource("classes/foo/temporary_filters/42/")) //
				.setParameter(LIMIT, "123") //
				.setParameter(START, "456") //
				.build());
		final HttpResponse response = httpclient.execute(request);
	...
	public void update() throws Exception {
		// when
		final HttpPut request = new HttpPut(server.resource("classes/foo/temporary_filters/42/"));
		request.setEntity(new StringEntity("{}", APPLICATION_JSON));
		final HttpResponse response = httpclient.execute(request);
	...
	public void delete() throws Exception {
		// when
		final HttpDelete request = new HttpDelete(server.resource("classes/foo/temporary_filters/42/"));
		final HttpResponse response = httpclient.execute(request);
